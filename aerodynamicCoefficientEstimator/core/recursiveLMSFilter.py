...
# there will be an independent stationary kalman filter class, or  instantiated for every
# input y into our system. Each single variable sensor input to our system will have


# Reminder for the structure, we are estimatoring x in the equation Ax = b
# where A, and b are known. The rows of A are transpose(a_n)
# and the rows of b are transpose(y_n)


class recursiveLMSFilter:
    ...
    # returns
    def update(self, y_n: np.ndarray, a_n: np.ndarray):
        ...
        # or stationary Kalman Filter approach
            ...
            if self.recursive:
                ...
                K_N = (self.P @ a_n) @ np.linalg.inv(
                    np.eye(self.y_width) + np.transpose(a_n) @ self.P @ a_n
                )
                # gets the new P matrix
                self.P = self.P - K_N @ a_n.T @ self.P
                # gets the update for x_star
                self.x_star = self.x_star + K_N @ (
                    y_n - np.transpose(a_n) @ self.x_star
                )
